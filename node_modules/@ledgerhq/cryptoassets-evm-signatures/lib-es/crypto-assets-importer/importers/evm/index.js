...
export const importTokenByChainId = async (outputDir, chainId, tokensOrderedByMarketCap) => {
    try {
...
`;
        if (erc20?.length && erc20Signatures?.length) {
            ...
            await fs.writeFile(path.join(dirPath, "erc20-signatures.json"), JSON.stringify(erc20Signatures.toString("base64")));
...
export const importEVMTokens = async (outputDir) => {
    ...
    await Promise.allSettled(supportedChainIds.map(chainId => importTokenByChainId(outputDir, chainId, tokens)));
