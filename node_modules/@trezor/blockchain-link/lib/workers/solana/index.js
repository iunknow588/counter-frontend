...
const getAccountInfo = async (request, isTestnet) => {
    ...
    if (details !== 'basic') {
        ...
        if (accountInfo) {
            ...
            const stakingData = await (0, utils_2.getSolanaStakingData)(payload.descriptor, isTestnet);
            misc = {
...
const getTokenSize = (programName) => ({ 'spl-token': (0, token_1.getTokenSize)(), 'spl-token-2022': (0, token_2022_1.getTokenSize)() })[programName];
const estimateFee = async (request) => {
    const api = await request.connect();
    const { data: messageHex, isCreatingAccount, newTokenAccountProgramName = 'spl-token', } = request.payload.specific ?? {};
    if (messageHex == null) {
