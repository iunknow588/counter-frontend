...
exports.toXOnly = exports.PubKeyBitcoinCompatible = exports.PubKeySecp256k1 = exports.PubKeyStarknet = exports.PrivKeySecp256k1 = void 0;
...
class PrivKeySecp256k1 {
    ...
    constructor(privKey, masterFingerprint, path) {
    ...
    toKeyPair() {
        return (0, ecpair_1.ECPairFactory)(ecc).fromPrivateKey(buffer_2.Buffer.from(this.privKey));
    }
    getPubKey() {
        return new PubKeySecp256k1(secp256k1_1.secp256k1.getPublicKey(this.privKey, true));
    }
    getBitcoinPubKey(network) {
        const pubKey = secp256k1_1.secp256k1.getPublicKey(this.toBytes(), false);
        return new PubKeyBitcoinCompatible(pubKey, network, this.masterFingerprint, this.path);
    }
    signDigest32(digest) {
        if (digest.length !== 32) {
...
class PubKeySecp256k1 {
    ...
    toBitcoinPubKey(network) {
        return new PubKeyBitcoinCompatible(this.toBytes(false), network, undefined, undefined);
    }
    /**
...
class PubKeyBitcoinCompatible {
    constructor(pubKey, network, masterFingerprint, path) {
        this.pubKey = pubKey;
    ...
    getBitcoinAddress(paymentType, network) {
        const pubKey = this.toBytes(false);
        const currentNetwork = network !== null && network !== void 0 ? network : this.network;
        const getLegacyAddress = () => {
            return bitcoinjs_lib_1.payments.p2pkh({
                pubkey: buffer_2.Buffer.from(pubKey),
        ...
        const getTaprootAddress = () => {
            return bitcoinjs_lib_1.payments.p2tr({
                internalPubkey: (0, exports.toXOnly)(buffer_2.Buffer.from(pubKey)),
                network: currentNetwork,
            }).address;
        };
        switch (paymentType) {
            ...
            default:
                ...
                if (path) {
                    ...
                    if (segments.length >= purposeIndex + 3) {
                        ...
                        const purpose = parseInt(purposeSegment.endsWith("'")
    ...
    static fromExtendedKey(xpub, basePath, masterFingerprint, additionalPath, network) {
        const root = (0, bip32_1.fromBase58)(xpub, network);
        ...
        if (additionalPath) {
            ...
            return new PubKeyBitcoinCompatible(child.publicKey, network, masterFingerprint, purposeIndex === 0
                ? `m/${basePath}/${additionalPath}`
                : `${basePath}/${additionalPath}`);
        }
        return new PubKeyBitcoinCompatible(root.publicKey, network, masterFingerprint, purposeIndex === 0 ? `m/${basePath}` : basePath);
    }
}
exports.PubKeyBitcoinCompatible = PubKeyBitcoinCompatible;
/**
