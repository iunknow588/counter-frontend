...
async function signAuthorization(client, parameters, organizationId, signWith) {
    ...
    const hashedAuthorization = utils.hashAuthorization({
        address: address,
        chainId,
        nonce,
    });
...
async function signTransaction(client, transaction, serializer, organizationId, signWith) {
    ...
    if (transaction.type === "eip4844") {
        ...
        const { r, s, v } = viem.parseTransaction(signedTx);
        // Recombine with the original transaction
        return viem.serializeTransaction(transaction, {
            r: r,
            s: s,
            v: v,
        });
...
async function signMessageImpl(client, message, organizationId, signWith, to) {
    ...
    return http.assertNonNull(serializeSignature(result, to));
}
...
function serializeSignature(sig, to = "hex") {
    ...
    const yParity_ = v;
    const signature = `0x${new secp256k1.secp256k1.Signature(viem.hexToBigInt(r), viem.hexToBigInt(s)).toCompactHex()}${yParity_ === 0n ? "1b" : "1c"}`;
    if (to === "hex")
        return signature;
    return viem.hexToBytes(signature);
}
